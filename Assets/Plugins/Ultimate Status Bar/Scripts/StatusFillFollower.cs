...
[ExecuteInEditMode]
[AddComponentMenu( "Ultimate UI/Status Fill Follower" )]
public class StatusFillFollower : MonoBehaviour
{
	...
	void OnStatusUpdated ( float currVal )
	{
		...
		if( Application.isPlaying == false )
		{
			// Check the fill constaints for the Ultimate Status. Since the user can be changing values, if the fill constraints are the same it will throw an error, so return.
